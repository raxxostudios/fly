...
class RevisionBrain(Brain):
    ...
    def step(self, luminance, duration_ms=20):
        ...
        if self.control_indices is not None:
            self.drive.fill(0)
            self.drive[self.output_indices]=self.config.get('output_tonic',0.)
            self.drive[self.control_indices]=self.control_level
        if self.silence_kc:
...
def interface_audit(kind='interface'):
    ...
    if kind=='relay-fine':
        configs=[dict(shared,id=f'fine_projection{proj}_kc{kc}',kc_tonic=kc,kc_input_gain=1.,projection_tonic=proj,optic_tonic=0.) for proj in [2.,3.,4.,5.] for kc in [6.4,6.8]]
...
def learn_local(brain, counts, target, selected):
    if selected==target:return 0
    eligibility=np.minimum(counts[brain.learn_pre]/20.,1.)
    change=np.zeros(len(brain.learn_edges),np.float32)
    change[brain.learn_class==target]=1.
    if selected is not None:change[brain.learn_class==selected]=-1.
    old=brain.weight[brain.learn_edges].copy()
    new=np.clip(old+.5*brain.learn_initial*eligibility*change,.01*brain.learn_initial,20.*brain.learn_initial)
    brain.weight[brain.learn_edges]=new
    return int(np.count_nonzero(old!=new))

def positive_control(replica=False):
    config={'id':'direct_KC_positive_control','sensor':'linear','visual_interface':True,'kc_tonic':0.,'output_tonic':6.5,'kc_recurrent_gain':.1,'output_input_gain':.1}
    suffix='-replication' if replica else ''
    base_seed=914320 if replica else 914300
    report={'id':'FLY-002-plasticity-control'+suffix,'created_at':datetime.datetime.now(datetime.timezone.utc).isoformat(),'kind':'direct_neural_stimulation_positive_control','source_sha256':sha(__file__),'protocol_sha256':sha(ROOT/'neural/PROTOCOL-002.md'),'seed_base':base_seed,'config':config,'learning_rate':.5,'training_trials':128,'held_out_trials_per_set':16,'pass_threshold_per_set':12,'citizenship_authorized':False,'records':[],'conditions':[]}
    b=RevisionBrain(config)
    ensembles=np.random.default_rng(base_seed).permutation(b.tonic_kc)[:64].reshape(4,16)
    report['stimulus_ensembles']=[[int(v) for v in row] for row in ensembles]
    report['stimulus_body_ids']=[[int(b.ids[v]) for v in row] for row in ensembles]
    initial=b.weight.copy();report['initial_sha256']=b.digest()
    for condition in ['untrained','trained','shuffled_labels']:
        b.weight[:]=initial
        rng=np.random.default_rng(base_seed+1);feedback_rng=np.random.default_rng(base_seed+2)
        if condition!='untrained':
            for block in range(8):
                sequence=np.tile(np.arange(4),4);rng.shuffle(sequence)
                for j,k in enumerate(sequence):
                    b.control_indices=ensembles[k];b.control_level=float(rng.uniform(18,22))
                    row,counts,_,_=trial(b,np.zeros(len(b.retina),np.float32),record=condition=='trained')
                    target=int(k) if condition=='trained' else int(feedback_rng.integers(4))
                    changed=learn_local(b,counts,target,row['selected'])
                    row.update(condition=condition,phase='training',stimulus=int(k),target=target,correct=row['selected']==int(k),changed_this_trial=changed,changed_weights=int(np.count_nonzero(b.weight!=initial)),drive=b.control_level,trial=block*16+j)
                    report['records'].append(row)
                save(OUT/f'plasticity-control{suffix}.json',report)
                batch=[r for r in report['records'] if r['condition']==condition and r['phase']=='training'][-16:]
                print(condition,'block',block+1,'correct',sum(r['correct'] for r in batch),'/16','changed',int(np.count_nonzero(b.weight!=initial)),flush=True)
        frozen=b.digest();scores=[]
        for seed in [base_seed+11,base_seed+12]:
            rng=np.random.default_rng(seed);sequence=np.tile(np.arange(4),4);rng.shuffle(sequence)
            correct=0
            for j,k in enumerate(sequence):
                b.control_indices=ensembles[k];b.control_level=float(rng.uniform(18,22))
                row,counts,_,_=trial(b,np.zeros(len(b.retina),np.float32),record=condition=='trained')
                row.update(condition=condition,phase='held_out',seed=seed,trial=j,stimulus=int(k),target=int(k),correct=row['selected']==int(k),drive=b.control_level,changed_weights=int(np.count_nonzero(b.weight!=initial)))
                correct+=int(row['correct']);report['records'].append(row)
            scores.append(correct)
            assert b.digest()==frozen,'Frozen control weights changed'
        result={'condition':condition,'scores':scores,'weight_sha256':frozen,'weights_frozen':True,'changed_weights':int(np.count_nonzero(b.weight!=initial))}
        report['conditions'].append(result)
        if condition=='trained':
            np.savez_compressed(OUT/f'plasticity-control{suffix}-weights.npz',edges=b.learn_edges,weight=b.weight[b.learn_edges],initial=b.learn_initial)
        save(OUT/f'plasticity-control{suffix}.json',report)
        print(json.dumps(result),flush=True)
    trained=next(x for x in report['conditions'] if x['condition']=='trained')
    report['positive_control_passed']=all(n>=12 for n in trained['scores'])
    report['visual_gate_passed']=False
    report['conclusion']='Direct-stimulation control only; the retinal visual-learning gate remains unpassed.'
    save(OUT/f'plasticity-control{suffix}.json',report)

def verify_control():
    report=json.loads((OUT/'plasticity-control.json').read_text())
    b=RevisionBrain(report['config']);checkpoint=np.load(OUT/'plasticity-control-weights.npz')
    original=b.weight.copy();b.weight[checkpoint['edges']]=checkpoint['weight']
    expected=next(x for x in report['conditions'] if x['condition']=='trained')['weight_sha256']
    assert b.digest()==expected,'Checkpoint hash mismatch'
    trained=b.weight.copy();ensembles=np.array(report['stimulus_ensembles']);results=[]
    for condition in ['checkpoint_replay','learned_weights_reset','KC_silenced']:
        b.weight[:]=original if condition=='learned_weights_reset' else trained
        b.silence_kc=condition=='KC_silenced';frozen=b.digest();records=[]
        for seed in [914311,914312]:
            rng=np.random.default_rng(seed);sequence=np.tile(np.arange(4),4);rng.shuffle(sequence)
            for j,k in enumerate(sequence):
                b.control_indices=ensembles[k];b.control_level=float(rng.uniform(18,22))
                row,_,_,_=trial(b,np.zeros(len(b.retina),np.float32))
                row.update(seed=seed,trial=j,target=int(k),correct=row['selected']==int(k));records.append(row)
        assert b.digest()==frozen
        if condition=='checkpoint_replay':
            prior=[r for r in report['records'] if r['condition']=='trained' and r['phase']=='held_out']
            assert all(a['selected']==c['selected'] and a['total_spikes']==c['total_spikes'] and a['rates_hz']==c['rates_hz'] for a,c in zip(prior,records))
        result={'condition':condition,'score':sum(r['correct'] for r in records),'trials':len(records),'weight_sha256':frozen,'records':records}
        results.append(result);print(condition,result['score'],'/32',flush=True)
    save(OUT/'verification.json',{'id':'FLY-002-positive-control-verification','source_sha256':sha(__file__),'protocol_sha256':sha(ROOT/'neural/PROTOCOL-002.md'),'checkpoint_sha256':sha(OUT/'plasticity-control-weights.npz'),'results':results,'replayed_exact_spike_counts':True})

if __name__=='__main__':
    {'audit':audit,'interface-audit':interface_audit,'relay-audit':lambda:interface_audit('relay'),'relay-fine-audit':lambda:interface_audit('relay-fine'),'positive-control':positive_control,'replicate-control':lambda:positive_control(True),'verify-control':verify_control}[sys.argv[1]]()
