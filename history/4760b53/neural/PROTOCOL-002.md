...
## Amendment C · resolve the transition between silence and broad activation

Relay current 6 produced broad or excessive baseline KC responses, while relay
current 0 produced silence in the preceding audit. Before learning, test the
intermediate relay currents 2, 3, 4, 5, each with visual KC tonic 6.4 then 6.8.
Keep optic tonic zero, external KC gain 1 and the other Amendment A settings.
Use the unchanged calibration criteria and report all eight configurations.
This searches excitability using calibration stimuli only. Held-out training
performance has not been observed or used to select any model parameters.

## Amendment D · separate positive control of synaptic learning

None of the 22 calibration configurations met its stated admission criteria.
The retinal learning gate therefore remains unpassed and citizenship training
remains blocked. To isolate the plasticity mechanism, conduct an explicitly
separate direct-neural-stimulation positive control. It does not replace the
retinal task or authorize citizenship training, whatever its result.

Using seed 914300, divide 64 randomly selected KCg-d cells into four disjoint
ensembles of 16. Activate one ensemble with uniform current sampled from 18–22
for 400 ms; retinal and other sensory drive are zero. Set visual KC tonic zero,
output tonic 6.5, KC recurrence gain 0.1, output input gain 0.1 and the anatomy-
selected MBON09/27/05/01 decoder. All other retained graph edges remain present.

Training consists of 128 trials in eight balanced shuffled blocks, seed 914301.
For an incorrect/abstained trial, update existing KC-to-output efficacy by
`0.5 × initial efficacy × min(pre_count/20,1)`, increasing target edges and
decreasing selected-wrong edges. Clip to 0.01–20 times initial efficacy. Target
values enter only the update. This is supervised local engineering plasticity,
not a reproduction of dopamine biochemistry or the Google paper's experiment.

Compare unchanged weights, true-label training and independent random-label
training (feedback seed 914302). Evaluate two balanced sets of 16 with fresh
current amplitudes, seeds 914311 and 914312; freeze and hash weights before each
evaluation. Twelve correct on each set indicates success of this control only.
Report all three conditions and all trials. Even a successful positive control
would leave the separate visual-learning and citizenship questions unresolved.
