...
def positive_control():
    ...
    report={'id':'FLY-002-plasticity-control','created_at':datetime.datetime.now(datetime.timezone.utc).isoformat(),'kind':'direct_neural_stimulation_positive_control','source_sha256':sha(__file__),'protocol_sha256':sha(ROOT/'neural/PROTOCOL-002.md'),'config':config,'learning_rate':.5,'training_trials':128,'held_out_trials_per_set':16,'pass_threshold_per_set':12,'citizenship_authorized':False,'records':[],'conditions':[]}
    b=RevisionBrain(config)
    ensembles=np.random.default_rng(914300).permutation(b.tonic_kc)[:64].reshape(4,16)
    ...
    for condition in ['untrained','trained','shuffled_labels']:
        b.weight[:]=initial
        rng=np.random.default_rng(914301);feedback_rng=np.random.default_rng(914302)
        if condition!='untrained':
            for block in range(8):
                ...
                for k in sequence:
                    ...
                    row.update(condition=condition,phase='training',stimulus=int(k),target=target,correct=row['selected']==int(k),changed_this_trial=changed,changed_weights=int(np.count_nonzero(b.weight!=initial)),drive=b.control_level,trial=block*16+int(np.flatnonzero(sequence==k)[0]))
                    report['records'].append(row)
                save(OUT/'plasticity-control.json',report)
        ...
        if condition=='trained':
            np.savez_compressed(OUT/'plasticity-control-weights.npz',edges=b.learn_edges,weight=b.weight[b.learn_edges],initial=b.learn_initial)
        save(OUT/'plasticity-control.json',report)
...
if __name__=='__main__':
    {'audit':audit,'interface-audit':interface_audit,'relay-audit':lambda:interface_audit('relay'),'relay-fine-audit':lambda:interface_audit('relay-fine'),'positive-control':positive_control}[sys.argv[1]]()
