...
def interface_audit():
    ...
    report={'id':'FLY-002-interface-audit','created_at':datetime.datetime.now(datetime.timezone.utc).isoformat(),'source_sha256':sha(__file__),'protocol_sha256':sha(ROOT/'neural/PROTOCOL-002.md'),'configs':configs,'records':[],'selected_config':None}
    ...
    for cfg in configs:
        ...
        for label in ['blank',0,1,2,3]:
            ...
            np.savez_compressed(OUT/f'interface-{cfg["id"]}-{label}.npz',counts=total,vmax=vmax,gmax=gmax)
            save(OUT/'interface-audit.json',report)
...
if __name__=='__main__':
    {'audit':audit,'interface-audit':interface_audit}[sys.argv[1]]()
